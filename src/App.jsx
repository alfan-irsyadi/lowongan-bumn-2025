    ...
    const skor = Math.round(
      ((streamWeight[vacancy.stream_name] +
        (vacancy.total_applied && !isNaN(vacancy.total_applied)
          ? 1 - vacancy.total_applied / vacancy.total_quota
          : 0) *
          5 +
        employWeight[vacancy.employment_status]) /
        15) *
        100,
      2
    );
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        const majorMap = Object.fromEntries(
          majorData.map((item) => [item.vacancy_id, item])
        );
        // let education = {};
        let educationMap = Object.fromEntries(
          educationData.map((item) => [item.vacancy_id, []])
        );

        educationData.map((item) => {
          educationMap[item.vacancy_id].push(item);
        });
  ...
  return (
    <div className="flex flex-col w-screen">
      <div className="header">
        ...
        <img
          src="https://pkg-rbb.fhcibumn.id/navbar-rbb/_/_/_/_/public/assets/images/portal/home/bumn_untuk_indonesia.png"
          alt=""
          srcset=""
          height={"30px"}
      ...
      <div className="flex flex-row w-screen max-h-[calc(100vh-4rem)] md:max-h-[calc(100vh-1rem)] overflow-y-auto pt-14">
        ...
        <div className={`flex flex-col h-full w-full float-left p-4`}>
          {notification && (
            <div className="bg-blue-100 p-4 text-gray-700 mb-4">
              {notification}
            </div>
          )}
            ...
            <div className="max-w-5xl mx-auto p-6 bg-[#ecf0f3] shadow-lg rounded-lg bevel">
              <div className="emboss">
                ...
                <div className="flex items-center space-x-3 mb-2">
                  <img
                    src={selectedVacancy.company_logo}
                    alt="Company Logo"
                    className="h-10"
                  />
                ...
                <div className="flex space-x-4 text-sm text-gray-500 mt-2">
                  <span>
                    📍{" "}
                    {eval(selectedVacancy.placement_region_name).join(", ") ||
                      "Seluruh Indonesia"}
                  </span>
              ...
              <div className="bg-gray-100 p-4 rounded-lg mt-6">
                ...
                <div className="flex items-center space-x-2 text-gray-700 mt-2">
                  ...
                  <span>
                    {
                      selectedVacancy.qualifications.education_level[0]
                        .education_level
                    }{" "}
                    - Min. IPK:{" "}
                    {
                ...
                <div className="flex items-center space-x-2 text-gray-700 mt-2">
                  <span>🚻</span>
                  <span>
                    {selectedVacancy.qualifications.gender === "male"
                      ? "Laki-laki"
                      : "Perempuan"}
                  </span>
                </div>
              ...
              <div className="bg-gray-100 p-4 rounded-lg mt-4">
                ...
                <ul className="list-disc list-inside text-gray-700">
                  {selectedVacancy.major?.major_type === "notin"
                    ...
                    ? "Semua jurusan"
                    : ""}
                  {selectedVacancy.qualifications.major.list_major.map(
                    (major, index) => (
                ...
                <div className="space-y-6 mt-4">
                  {vacancies.map((vacancy, index) => (
                    <div
                      ...
                      className="bg-white px-4 text-xs/8 py-8 shadow-lg rounded-lg border border-gray-200 relative neo-button"
                      onClick={() => handleVacancyClick(vacancy)}
                    >
                      <div className="flex justify-between">
                        <div>
                          <h2 className="text-lg font-bold">{`${index + 1}. ${
                            vacancy.title
                          }`}</h2>
                      ...
                      <div className="mt-2 text-xs/6 text-gray-700">
                        ...
                        <div>
                          {vacancy.education.map((item) => {
                            return (
                              <pre>
                                <p>
                                🏫 {" "}{item.education_level} : <br />
                                  {"\t"}📜 {" "}Min IPK: {item.score_min || "-"} dan 🕑 Max
                                  Usia: {item.age_max} Tahun
                                </p>
                              </pre>
                            );
                          })}
                        ...
                        {vacancy.education?.age_max && (
                          <p>🕑 Max Usia: {vacancy.education.age_max} Tahun</p>
                        )} */}
                      </div>
                      ...
                      <div className="mt-2 text-xs/6">
                        ...
                        {vacancy.major?.major_type === "notin"
                    ? `🎓 Semua jurusan dapat melamar, kecuali:`
                    : 
                     "🎓 Semua jurusan dapat melamar:"
                    } 
                        </p>
                        <p className="text-gray-700 max-h-36 overflow-y-auto">
                          {vacancy.major?.list_major || "Semua Jurusan"}
                        </p>
                      </div>
                      ...
                      <div className="flex my-4 flex-row justify-between">
                        <div className="flex gap-2 mt-3">
                          <span className="bg-blue-200 text-blue-800 px-3 py-1 text-xs rounded-full">
                            Pegawai Tetap
                          </span>
                          {vacancy.stream_name && (
                        ...
                        <div>
                          <span className="text-xs">
                            {vacancy.total_applied} / {vacancy.total_quota} (
                            {parseInt(
                              (vacancy.total_applied / vacancy.total_quota) *
                                100
                            ) + "%"}
